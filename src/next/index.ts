...
import {
  corsHeaders,
  fetchFusionAuthAuthorizationServerMetadata,
  generateFusionAuthProtectedResourceMetadata,
  generateProtectedResourceMetadata,
  verifyFusionAuthToken,
} from "../server";
...
export function protectedResourceHandlerFusionAuth(
  ...
  return (req: Request) => {
    ...
    const authServerUrl = process.env.NEXT_PUBLIC_FUSIONAUTH_AUTHORIZATION_SERVER_URL;
    if (!authServerUrl) {
      throw new Error(
        "Missing NEXT_PUBLIC_FUSIONAUTH_AUTHORIZATION_SERVER_URL environment variable"
      );
    }
 ...
 */
export function authServerMetadataHandlerFusionAuth() {
  return async () => {
    const authServerUrl = process.env.NEXT_PUBLIC_FUSIONAUTH_AUTHORIZATION_SERVER_URL;
    if (!authServerUrl) {
      throw new Error(
...
//TOODO: Don't love this, should we use a library?
// re-export the verifyFusionAuthToken function for convenience
export { verifyFusionAuthToken };
