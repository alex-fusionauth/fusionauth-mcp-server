...
/**
 * Express middleware that enforces authentication for MCP requests and automatically verifies the OAuth access token using FusionAuth.
 * @example
 * ```ts
 * const server = new McpServer({
 *   name: "test-server",
 ...
 * // define server tools, resources, etc...
 *
 * app.get("/mcp", mcpAuthFusionAuth, streamableHttpHandler(server));
 * ```
 */
export async function mcpAuthFusionAuth(
  req: express.Request,
  res: express.Response,
  next: express.NextFunction
): Promise<void> {
...
/**
 * Verifies the access token with FusionAuth.
 * @param token - The access token to verify
 * @returns True if the token is valid, false otherwise
 */
async function verifyAccessToken(token: string) {
  return await client.validateJWT(token);
}